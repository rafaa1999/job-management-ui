...
export class CounterService {
  ...
  getAllCountersByFacilityId(id:string){
    ...
    return this.http.get(`http://localhost:8081/api/counters/facility/${id}`);
  }
  ...
  simulateAdd(id: any){
    ...
    return this.http.get(`http://localhost:8081/scheduler/simulation/add/${id}`)    
    .pipe(map(resData => resData));
  ...
  simulateDelete(id: any){
    ...
    // .pipe(map(resData => resData));
    return this.http.get(`http://localhost:8081/scheduler/simulation/delete/${id}`)    
    .pipe(map(resData => resData));
    // return this.http.get(`http://localhost:8081/scheduler/simulation/${id}/?tenantId=meta`, { headers: this.headers })
  }
